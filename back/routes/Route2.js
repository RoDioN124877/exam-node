...
const D2players2 = require("../models/D2players2");
...
router.get("/", async (req, res) => {
  ...
  const numberOfItems = await D2players2.countDocuments();

  const totalPages = Math.ceil(numberOfItems / limit);

  const d2players = await D2players2.find()
    .skip(skipNumberOfItems)
    .limit(limit);
